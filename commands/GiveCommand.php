...
require_once 'api/imageApi.php'; // Adjust as needed
...
class GiveCommand
{
    ...
    public function execute(Message $message)
    {
        ...
        // Check for receiver user, item, and amount with the new prefix
        if (preg_match('/^m\.give\s+(\S+)\s+(.+)\s+(\d+)$/', $content, $matches)) {
            ...
            $message->channel->sendMessage("Usage: `m.give <username> <item> <amount>`");
        }
    }
